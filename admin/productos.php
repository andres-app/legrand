...
$currentSaleOrder = (int)($current['sale_order'] ?? 0);
...
foreach ($products as $productItem) {
    ...
    // Si estoy editando el mismo producto, no cuento su propia posición
    if ($currentSlug !== '' && $productSlug === $currentSlug) {
        continue;
    }

    if (!empty($productItem['sale_featured'])) {
        $order = (int)($productItem['sale_order'] ?? 0);

        if ($order >= 1 && $order <= 5) {
            $usedSaleOrders[] = $order;
        }
    }
}

$availableSaleOrders = array_values(array_diff(range(1, 5), $usedSaleOrders));

if ($currentSaleOrder >= 1 && $currentSaleOrder <= 5 && !in_array($currentSaleOrder, $availableSaleOrders, true)) {
    $availableSaleOrders[] = $currentSaleOrder;
}

sort($availableSaleOrders);

$canSelectSalePosition = count($availableSaleOrders) > 0;
                    ...
                    <div class="grid gap-6 lg:grid-cols-[320px_1fr]">
                        ...
                        <div class="grid gap-5">
                            ...
                            <div class="rounded-[1.4rem] border border-amber-100 bg-amber-50/60 p-4">
                                <label class="flex items-center gap-3 font-black text-neutral-800">
                                    <input
                                        type="checkbox"
                                        name="sale_featured"
                                        value="1"
                                        class="h-4 w-4 accent-[#C7962E]"
                                        <?= !empty($current['sale_featured']) ? 'checked' : ''; ?>
                                        <?= !$canSelectSalePosition && empty($current['sale_featured']) ? 'disabled' : ''; ?>>
                                    Mostrar en “Rebajas disponibles”
                                </label>

                                <div class="mt-4">
                                    <label class="text-sm font-black text-neutral-700">Orden en rebajas</label>

                                    <?php if ($canSelectSalePosition): ?>
                                        <select
                                            name="sale_order"
                                            class="select-caret field-focus mt-2 w-full appearance-none rounded-2xl border border-amber-100 bg-white px-4 py-3 pr-10 text-sm font-bold outline-none">
                                            <?php foreach ($availableSaleOrders as $position): ?>
                                                <option
                                                    value="<?= $position; ?>"
                                                    <?= ((int)($current['sale_order'] ?? 0) === $position) ? 'selected' : ''; ?>>
                                                    Posición <?= $position; ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                    <?php else: ?>
                                        <div class="mt-2 rounded-2xl border border-red-100 bg-white px-4 py-3 text-sm font-black text-red-700">
                                            Ya están ocupadas las 5 posiciones disponibles.
                                        </div>
                                    <?php endif; ?>
                                </div>

                                <p class="mt-3 text-xs font-bold leading-5 text-amber-700">
                                    Solo se muestran posiciones libres. Máximo 5 productos en “Rebajas disponibles”.
                                </p>
                            </div>
                        </div>
                    </div>
